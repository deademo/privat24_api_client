...
parser.add_argument('-hi', '--history', action='store_true', help='Will get and show ballance of card')
...
def main(args):
    ...
    if args.history:
        history = list(api.history(args.card_number, from_date=args.from_date, to_date=args.to_date))
        for item in history:
